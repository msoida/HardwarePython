...
class BMP280(object):
    ...
    I2C_ADDRESS = 0x77
    I2C_ADDRESS2 = 0x76 # Not used in this implementation
    ...
    def calibrate(self):
        ...
        data = self._bus.read_i2c_block_data(self.I2C_ADDRESS,
            self.CALIBRATION, 24)
    ...
    def id(self):
        ...
        return self._bus.read_byte_data(self.I2C_ADDRESS, self.ID)

    def reset(self):
        """Reset all internal registers."""
        self._bus.write_byte_data(self.I2C_ADDRESS, self.RESET,
            self.SOFT_RESET)

    @property
    def status(self):
        """Return current device status as tuple (measuring, im_update)."""
        data = self._bus.read_byte_data(self.I2C_ADDRESS, self.STATUS)
    ...
    def ctrl_meas(self):
        ...
        return self._bus.read_byte_data(self.I2C_ADDRESS, self.CTRL_MEAS)

    def _ctrl_meas_set(self, osrs_t, osrs_p, mode):
        """Set ctrl_meas register (for internal use)."""
        data = (osrs_t << 5) + (osrs_p << 2) + mode
        self._bus.write_byte_data(self.I2C_ADDRESS, self.CTRL_MEAS,
            data)
    ...
    def config(self):
        ...
        return self._bus.read_byte_data(self.I2C_ADDRESS, self.CONFIG)

    def _config_set(self, t_sb, _filter, spi3w_en):
        """Set config register (for internal use)."""
        data = (t_sb << 5) + (_filter << 2) + spi3w_en
        self._bus.write_byte_data(self.I2C_ADDRESS, self.CONFIG, data)

    def set_config(self, inactive_time, filter_constant, spi_3wire=None):
        """Set configuration options."""
        it = inactive_time
        fc = filter_constant
        s3 = spi_3wire
        if (it is None) or (it == 0) or (it == 1):
        ...
        if (spi_3wire is None) or (spi_3wire == False) or (spi_3wire == 0):
            spi3w_en = 0x00
        elif (spi_3wire == True) or (spi_3wire == 1):
            spi3w_en = 0x01
        else:
            raise BMP280Error('Unknown SPI mode')

        self._config_set(t_sb, _filter, spi3w_en)

    def raw_pressure(self):
        """Return measured pressure (raw data)."""
        data = self._bus.read_i2c_block_data(self.I2C_ADDRESS, self.PRESS, 3)
        return (data[0] << 12) + (data[1] << 4) + (data[2] >> 4)

    def raw_temperature(self):
        """Return measured temperature (raw data)."""
        data = self._bus.read_i2c_block_data(self.I2C_ADDRESS, self.TEMP, 3)
        return (data[0] << 12) + (data[1] << 4) + (data[2] >> 4)
